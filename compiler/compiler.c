...
static int declareLocalVar(CompileUnit* cu, const char* name, uint32_t length) {
    ...
    while (index >= 0) {
      ...
      if (var->length == length && memcmp(var->vame, name, length) == 0) {
        char id[MAX_ID_LEN] = {'\0'};
        memcpy(id, name, length);
...
//为单运算符方法创建签名
static void unaryMethodSignature(CompileUnie* cu UNUSED, Signature* sign UNUSED) {
  //名称部分调用前已经完成，只修改类型
...
static CompileUnit* getEnclosingClassBK(CompileUnit* cu) {
    while(cu != NULL) {
...
//为既做单运算符有做中运算符的符号方法创建签名
static void mixMethodSignature(CompileUnie* cu, Signature* sign) {
...
static void condition(CompileUnit* cu, bool canAssign UNUSED) {
    ...
    uint32_t falseBrachEnd = emitInstrWithPlaceholder(cu, OPCODE_JUMP);
...
static void leaveLoopPatch(CompileUnit* cu) {
    ...
    //循环体结束地址
    uint32_t loopEndindex = cu->fn->instrStream;
...
static void compileForStatment(CompileUnit* cu) {
  ...
  writeByteOperand(cu, OPCODE_STORE_LOCAL_VAR, iterSlot);
