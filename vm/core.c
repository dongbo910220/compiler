...
//uses Boyer-Moore-Horspool字符串匹配算法在haystack中查找needle,大海捞针
static int findString(ObjString* haystack, ObjString* needle) {
    ...
    while (idx <= range) {
      ...
      //(因为Boyer-Moore-Horspool是从后往前比较), 如果匹配,看整个n
...
#define PRIM_NUM_BIT(name, operator) \
 ...
 return false;\
    ...
    int32_t leftOperand = VALUE_TO_NUM(args[0]); \
...
static bool primStringSubscript(VM* vm, Value* args) {
    ...
    if (VALUE_IS_NUM(args[1])) {
      uint32_t index = (vm, args[1], objString->value.length);
      if (index == UINT32_MAX) {
	       return false;
      }
...
void buildCore(VM* vm) {
  ...
  PRIM_METHOD_BIND(vm->fnClass->objHeaeder.class, "new(_)", primFnNew);
